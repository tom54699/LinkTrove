...
function makeSvc(initial: any[]) {
  ...
  const svc = createWebpageService({
    storage: {
      saveToLocal: async (d: any[]) => { store.splice(0, store.length, ...d); },
      loadFromLocal: async () => [...store],
      saveToSync: async () => {}, loadFromSync: async () => [],
      saveTemplates: async () => {}, loadTemplates: async () => [],
      exportData: async () => '[]', importData: async () => {},
    } as any,
  });
  return { svc, store };
}
...
describe('reorderWebpages insert before target', () => {
  it('inserts at target index and shifts target forward', async () => {
    const initial = [
      { id: 'n', title: 'New', url: 'https://n', favicon:'', note:'', category:'default', createdAt:'', updatedAt:'' },
      { id: 'a', title: 'A', url: 'https://a', favicon:'', note:'', category:'default', createdAt:'', updatedAt:'' },
    ];
