        ...
        <div className="bg-gradient-to-r from-[#181c2266] to-[#282a3666] border border-[var(--border)] rounded-xl p-5 mb-6">
          <div className="flex gap-3 mb-4">
            ...
            <div>
              ...
              <div className="text-[12px] text-[var(--muted)]">從空白開始，或選擇預設</div>
            </div>
          </div>
              ...
              <div key={t.id} className={`rounded-xl border transition-all overflow-hidden bg-[var(--surface)] ${isOpen ? 'border-[var(--muted)] shadow-xl' : 'border-[var(--border)] hover:border-[var(--muted)]/40'}`}>
                ...
                <div className={`p-3 pl-4 flex items-center justify-between cursor-pointer hover:bg-black/20 bg-black/10 ${isOpen ? 'border-b border-[var(--border)]' : ''}`} onClick={() => setExpanded(prev => ({ ...prev, [t.id]: !prev[t.id] }))}>
                  ...
                  <div onClick={e => e.stopPropagation()}>
                    {usageMap[t.id] > 0 ? <div className="p-1.5 opacity-30 cursor-not-allowed text-[var(--muted)]">🔒</div> : (
                      <button className="p-1.5 rounded text-[var(--muted)] hover:text-red-400 cursor-pointer transition-all" onClick={() => actions.remove(t.id)}>
                    ...
                    <div className="space-y-2">
                      {(t.fields || []).map((f) => (
                        <div key={f.key} className="grid grid-cols-[30px_2fr_2fr_1.5fr_60px_60px] gap-2.5 items-center p-2 bg-black/20 border border-[var(--border)] rounded-md group hover:border-[var(--muted)]/30 transition-colors">
                          ...
                          <input className="bg-black/20 border border-[var(--border)] rounded px-2 py-1 text-[13px] text-[var(--fg)] outline-none focus:border-[var(--accent)] transition-all" value={f.label} onChange={e => actions.updateField(t.id, f.key, { label: e.target.value })} />
                          <div className="text-[12px] opacity-70 px-1 text-[var(--muted)]">{f.type}</div>
                          <div className="text-center"><input type="checkbox" checked={!!f.required} onChange={e => actions.updateFieldRequired(t.id, f.key, e.target.checked)} className="accent-[var(--accent)]" /></div>
                          <div className="text-center"><button className="text-red-400 opacity-0 group-hover:opacity-100 cursor-pointer hover:text-red-300 transition-all" onClick={() => actions.removeField(t.id, f.key)}>×</button></div>
                        </div>
                      ))}
                    </div>
                    <div className="mt-4 bg-[var(--bg)]/50 border border-[var(--border)] rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-[2fr_2fr_1.5fr_auto] gap-2.5 items-end">
                        ...
                        <button className="px-4 py-1.5 rounded bg-[var(--accent)] text-white text-[12px] font-bold cursor-pointer hover:brightness-110 active:scale-95 shadow-sm transition-all" onClick={async () => {
                          const nf = newField[t.id]; if (!nf?.key || !nf?.label) return;
                          await actions.addField(t.id, { ...nf, type: nf.type || 'text' });
                          setNewField({ ...newField, [t.id]: {} });
                          showToast('欄位已新增', 'success');
                        }}>Add</button>
                      </div>
                    </div>
                  </div>
      ...
      {modal && (
        <div className="fixed inset-0 z-[9999] bg-black/70 flex items-center justify-center p-3 backdrop-blur-sm" onClick={() => setModal(null)}>
          <div className="rounded-xl border border-[var(--border)] bg-[var(--panel)] w-[420px] max-w-[95vw] p-5 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex justify-end"><button className="px-5 py-2 rounded-lg border border-[var(--border)] text-[var(--fg)] hover:bg-white/5 cursor-pointer transition-all text-[13px]" onClick={() => setModal(null)}>知道了</button></div>
          </div>
        </div>
      )}
    </div>
