...
- [ ] 2.2 實作資料驗證和完整性檢查
    - [x] 建立遷移前後的資料完整性驗證（數量校驗，返回錯誤清單與 skipped）
    - [x] 實作重複資料檢測和處理機制（依 URL 規範化，keep-first）
    - [x] 加入遷移失敗的回滾功能（transaction 內寫入，失敗回滾）
    - [x] 建立遷移報告（MigrationResult: bookmarks/categories/skipped/errors）
    - _需求: 2.4, 2.5_

- [ ] 3. 實作核心資料服務
  - [ ] 3.1 建立 BookmarkService CRUD 操作
    - [x] 實作 BookmarkService 類別的基本 CRUD 功能
    - [x] 建立書籤建立、讀取、更新、刪除的資料庫操作
    - [x] 實作批次操作功能 (批次新增、刪除)
    - [x] 加入資料驗證和錯誤處理機制（URL 檢查、事務包裝）
    - _需求: 1.2, 1.3_

  - [ ] 3.2 實作 CategoryService 階層管理
    - [x] 建立 CategoryService 處理分類的階層結構
    - [x] 實作分類的建立、更新、刪除和重新排序
    - [x] 建立分類樹狀結構的查詢和操作功能
    - [x] 實作分類統計和使用情況分析（direct/withChildren 計數）
    - _需求: 5.1, 5.3, 5.4, 5.5_

  - [ ] 3.3 實作 TagService 標籤系統
    - [x] 建立 TagService 處理標籤的 CRUD 操作
    - [x] 實作書籤與標籤的多對多關聯管理
    - [x] 建立標籤的顏色設定和視覺化功能
    - [x] 實作標籤使用統計和自動建議功能（stats/suggest）
    - _需求: 5.2, 5.4_

- [ ] 4. 實作搜尋引擎功能
  - [ ] 4.1 建立全文搜尋功能
    - [x] 實作 SearchEngine 類別使用 SQLite FTS5（由 DB 管理層自動 FTS/LIKE 切換）
    - [x] 建立全文搜尋的查詢介面和結果排序（title/desc/url 權重）
    - [x] 實作搜尋結果的關鍵字高亮顯示
    - [x] 加入搜尋效能優化和快取機制（LRU 20 筆）
    - _需求: 3.1, 3.3, 3.5_

  - [ ] 4.2 實作進階搜尋和篩選
    - [x] 建立進階搜尋的多條件查詢功能（分類/含子樹、標籤、日期、排序）
    - [x] 實作按分類、標籤、日期範圍的篩選
    - [x] 建立搜尋建議和自動完成功能
    - [x] 實作搜尋歷史的記錄和管理（in-memory）
    - _需求: 3.2, 3.4_
...
- [ ] 7. 實作安全性和資料保護
  - [ ] 7.1 建立資料加密功能
    - [x] 可選的端到端加密（Encryptor：AES-GCM，PassphraseBox）
    - [x] 同步資料加密（FileSystemSync/CloudSync 皆可注入 Encryptor）
    - [ ] 本地資料庫加密（SQLite 加密需特定 wasm 版本；待後續集成）
    - [x] 加密金鑰管理介面（可注入；範例 PassphraseBox，實務可存 storage）
    - _需求: 7.1, 7.4_

  - [ ] 7.2 實作備份和恢復系統
    - [x] 手動備份（BackupManager.writeBackup）與 SHA-256 完整性檢驗
    - [x] 備份驗證與還原（verify/restore）
    - [ ] 自動備份整合（可由 AutoSync/排程觸發；待 UI）
    - [ ] 資料損壞修復（需更深度檢測/重建策略；後續）
    - _需求: 7.2, 7.3_
