...
- **嚴格程式碼修改規範**：
  - **分析優先**：當使用者提及「分析」（analysis）時，**嚴禁修改程式碼**，必須保持純粹的分析與說明。
  - **先計畫後執行**：在修改任何程式碼之前，必須先提出詳細的**修改計畫（Plan）**與**程式碼預覽**。
  - **詢問許可**：嚴禁在回答問題後直接自動執行程式碼變更。必須明確詢問使用者（例如：「請問是否執行修改？」），並在獲得**明確同意**（如「可以」、「改吧」）後，才可執行 `write_file` 或 `replace`。
